...
object MessageIdentifiers {
    ...
    const val ID_OUT_OF_BAND_INTERNAL = 0x0d
    ...
    // / the message arrived
    const val ID_SND_RECEIPT_ACKED = 0x0e
    ...
    const val ID_SND_RECEIPT_LOSS = 0x0f
    ...
    const val ID_CONNECTION_LOST = 0x16
    ...
    const val ID_INVALID_PASSWORD = 0x18
    ...
    const val ID_IP_RECENTLY_CONNECTED = 0x1a
    ...
    // / the data parameter
    const val ID_ADVERTISE_SYSTEM = 0x1d
    ...
    const val ID_DOWNLOAD_PROGRESS = 0x1e
    ...
    // / NATPunchthrough plugin: internal
    const val ID_NAT_CONNECT_AT_TIME = 0x3b

    // / NATPunchthrough plugin: internal
    const val ID_NAT_GET_MOST_RECENT_PORT = 0x3c

    // / NATPunchthrough plugin: internal
    const val ID_NAT_CLIENT_READY = 0x3d
    // / NATPunchthrough plugin: internal
    ...
    const val ID_NAT_TARGET_NOT_CONNECTED = 0x3e
    ...
    const val ID_NAT_TARGET_UNRESPONSIVE = 0x3f
    ...
    const val ID_NAT_PUNCHTHROUGH_FAILED = 0x42
    ...
    const val ID_READY_EVENT_QUERY = 0x47

    // / Lobby packets. Second byte indicates type.
    const val ID_LOBBY_GENERAL = 0x48

    // RPC3, RPC4 error
    const val ID_RPC_REMOTE_ERROR = 0x49

    // / Plugin based replacement for RPC system
    const val ID_RPC_PLUGIN = 0x4a

    // / FileListTransfer transferring large files in chunks that are read only when needed, to save memory
    const val ID_FILE_LIST_REFERENCE_PUSH = 0x4b

    // / Force the ready event to all set
    const val ID_READY_EVENT_FORCE_ALL_SET = 0x4c

    // / Rooms function
    const val ID_ROOMS_EXECUTE_FUNC = 0x4d
    const val ID_ROOMS_LOGON_STATUS = 0x4e
    const val ID_ROOMS_HANDLE_CHANGE = 0x4f

    // / Lobby2 message
    const val ID_LOBBY2_SEND_MESSAGE = 0x50
    const val ID_LOBBY2_SERVER_ERROR = 0x51
    ...
    const val ID_FCM2_NEW_HOST = 0x52

    // / \internal For FullyConnectedMesh2 plugin
    const val ID_FCM2_REQUEST_FCMGUID = 0x53

    // / \internal For FullyConnectedMesh2 plugin
    const val ID_FCM2_RESPOND_CONNECTION_COUNT = 0x54

    // / \internal For FullyConnectedMesh2 plugin
    const val ID_FCM2_INFORM_FCMGUID = 0x55

    // / \internal For FullyConnectedMesh2 plugin
    const val ID_FCM2_UPDATE_MIN_TOTAL_CONNECTION_COUNT = 0x56
    ...
    // / \note Only the designated client gets this message
    const val ID_FCM2_VERIFIED_JOIN_FAILED = 0x59
    ...
    const val ID_FCM2_VERIFIED_JOIN_ACCEPTED = 0x5a
    ...
    // / \note Only the designated client gets this message
    // / \sa RespondOnVerifiedJoinCapable()
    const val ID_FCM2_VERIFIED_JOIN_REJECTED = 0x5b

    // / UDP proxy messages. Second byte indicates type.
    const val ID_UDP_PROXY_GENERAL = 0x5c

    // / SQLite3Plugin - execute
    const val ID_SQLite3_EXEC = 0x5d

    // / SQLite3Plugin - Remote database is unknown
    const val ID_SQLite3_UNKNOWN_DB = 0x5e

    // / Events happening with SQLiteClientLoggerPlugin
    const val ID_SQLLITE_LOGGER = 0x5f

    // / Sent to NatTypeDetectionServer
    const val ID_NAT_TYPE_DETECTION_REQUEST = 0x60

    // / Sent to NatTypeDetectionClient. Byte 1 contains the type of NAT detected.
    const val ID_NAT_TYPE_DETECTION_RESULT = 0x61

    // / Used by the router2 plugin
    const val ID_ROUTER_2_INTERNAL = 0x62
    ...
    const val ID_ROUTER_2_FORWARDING_NO_PATH = 0x63
    ...
    const val ID_ROUTER_2_FORWARDING_ESTABLISHED = 0x64
    ...
    const val ID_ROUTER_2_REROUTED = 0x65

    // / \internal Used by the team balancer plugin
    const val ID_TEAM_BALANCER_INTERNAL = 0x66
    ...
    const val ID_TEAM_BALANCER_REQUESTED_TEAM_FULL = 0x67
    ...
    const val ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED = 0x68
    const val ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED = 0x69
    ...
    const val ID_TEAM_BALANCER_TEAM_ASSIGNED = 0x6a

    // / Gamebryo Lightspeed integration
    const val ID_LIGHTSPEED_INTEGRATION = 0x6b

    // / XBOX integration
    const val ID_XBOX_LOBBY = 0x6c
    ...
    const val ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_SUCCESS = 0x6d
    const val ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_SUCCESS = 0x6e
    ...
    const val ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_FAILURE = 0x6f
    ...
    const val ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILURE = 0x70
    ...
    // / You can read the identifier used to challenge as follows:
    // / RakNet::BitStream bs(packet->data, packet->length, false); bs.IgnoreBytes(sizeof(MessageID)); RakNet::RakString password; bs.Read(password);
    const val ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT = 0x71

    // / \internal
    const val ID_TWO_WAY_AUTHENTICATION_NEGOTIATION = 0x72

    // / CloudClient / CloudServer
    const val ID_CLOUD_POST_REQUEST = 0x73
    const val ID_CLOUD_RELEASE_REQUEST = 0x74
    const val ID_CLOUD_GET_REQUEST = 0x75
    const val ID_CLOUD_GET_RESPONSE = 0x76
    const val ID_CLOUD_UNSUBSCRIBE_REQUEST = 0x77
    const val ID_CLOUD_SERVER_TO_SERVER_COMMAND = 0x78
    const val ID_CLOUD_SUBSCRIPTION_NOTIFICATION = 0x79

    // LibVoice
    const val ID_LIB_VOICE = 0x7a

    const val ID_RELAY_PLUGIN = 0x7b
    const val ID_NAT_REQUEST_BOUND_ADDRESSES = 0x7c
    const val ID_NAT_RESPOND_BOUND_ADDRESSES = 0x7d
    const val ID_FCM2_UPDATE_USER_CONTEXT = 0x7e
    const val ID_RESERVED_3 = 0x7f
    const val ID_RESERVED_4 = 0x80
    const val ID_RESERVED_5 = 0x81
    const val ID_RESERVED_6 = 0x82
    const val ID_RESERVED_7 = 0x83
    const val ID_RESERVED_8 = 0x84
    const val ID_RESERVED_9 = 0x85

    // For the user to use.  Start your first enumeration at this value.
    const val ID_USER_PACKET_ENUM = 0x86
    // -------------------------------------------------------------------------------------------------------------
}
